...
public class EsOperatorService {
	...
	private void processEmployeeOperator(TargetObject targetObject) {
		...
		if (OperatorTypeEnum.INSERT.equals(operator)) {
			...
			for (Employee employee : employees) {
				...
				List<Department> departments = departmentRepository.findDepartmentsByEmployeeId(employee.getId());
				for (Department department : departments) {
					this.deleteEmployeeFromTargetIndex(String.valueOf(department.getId()), DEPARTMENT_INDEX_NAME, String.valueOf(employee.getId()));
				}

				// 从所有的 product 文档中删除包含这个 employee 的数据
				List<Product> products = productRepository.findDepartmentsByEmployeeId(employee.getId());
				for (Product product : products) {
					this.deleteEmployeeFromTargetIndex(String.valueOf(product.getId()), PRODUCT_INDEX_NAME, String.valueOf(employee.getId()));
				}
			}
		}
	}
	...
	private void processProductOperator(TargetObject targetObject) {
		...
		if (OperatorTypeEnum.INSERT.equals(operator)) {
			...
			for (Department department : departments) {
				this.deleteProductFromTargetIndex(String.valueOf(department.getId()), DEPARTMENT_INDEX_NAME, String.valueOf(productId));
			}

			// 从所有的 employee文档中，删除包含这个 product 的数据
			List<Employee> employees = employeeRepository.findEmployeesByProductId(productId);
			for (Employee employee : employees) {
				this.deleteProductFromTargetIndex(String.valueOf(employee.getId()), EMPLOYEE_INDEX_NAME, String.valueOf(productId));
			}
		}
	}
	...
	private void addDepartmentToTargetIndex(String targetDocumentId, String targetIndexName, Object department) {
		...
		departmentParams.put("departments", Collections.singletonList(JSONObject.toJSON(department)));
